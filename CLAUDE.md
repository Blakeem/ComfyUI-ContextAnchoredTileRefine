...
ComfyUI custom node package, one tiling engine behind three nodes. They refine an
...
- **The frozen region is presented on a SCHEDULE, not re-noised** (`anchor_ring_factor` /
  ...
  override `scale_latent_inpaint`) are detected via `__mro__` and SKIPPED — their override is
  the endpoint this curve approaches. The patch goes on the model INSTANCE and is restored in
...
- Keep `grid.py` stdlib-only, and `node.py` / `sampling.py` / `conds.py` / `vl.py`
  module scopes comfy-free (lazy imports; `conds.py` never imports
